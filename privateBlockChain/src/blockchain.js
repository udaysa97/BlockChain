...
class Blockchain {
  ...
  _addBlock(block) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        reject("Error while ading block to chain");
      }
    });
  }
  ...
  requestMessageOwnershipVerification(address) {
    return new Promise((resolve) => {
      let ownerVerifyString = `${address}:${new Date()
        .getTime()
        .toString()
        .slice(0, -3)}:starRegistry`;
      resolve(ownerVerifyString);
    });
  }
  ...
  submitStar(address, message, signature, star) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        let timeInMessage = parseInt(message.split(":")[1]);
        let currentTime = parseInt(
          new Date().getTime().toString().slice(0, -3)
        );
        if (currentTime - timeInMessage > 300) {
          // check time difference in seconds (300 sec = 5 min)
          console.log("Inside check");
          reject("Time greater than 5 minutes");
        }
    ...
    return new Promise(async (resolve) => {
      self.chain.map(async (block) => {
        try {
          ...
          errorLog.push(`Block with hash ${block.hash} is tampered`);
        }
        if (
          block.height !== 0 &&
          ...
          errorLog.push(
            `Block with hash ${block.hash} has wrong previous block hash`
          );
        }
      });
      resolve(errorLog);
